...
object StLoadHomeAd {
        ...
        vpnNativeAds.withAdListener(object : AdListener() {
          ...
          """"
            ...
            override fun onAdLoaded() {
                super.onAdLoaded()
                KLog.d(logTagSt, "home---加载vpn原生广告成功")
            ...
            override fun onAdOpened() {
                super.onAdOpened()
                KLog.d(logTagSt, "home---点击vpn原生广告")
     ...
     * 设置展示vpn原生广告
     */
    fun setDisplayHomeNativeAdSt(activity: AppCompatActivity, binding: ActivityMainBinding) {
        activity.runOnUiThread {
            adBase.appAdDataSt?.let { adData ->
                if (adData is NativeAd && !adBase.whetherToShowSt && activity.lifecycle.currentState == Lifecycle.State.RESUMED) {
                    ...
                    binding.vpnAdSt = true
                    recordNumberOfAdDisplaysSt()
                    adBase.whetherToShowSt = true
                    App.nativeAdRefreshSt = false
                    adBase.appAdDataSt = null
                    KLog.d(logTagSt, "home--原生广告--展示")
                    //重新缓存
                    AdBase.getHomeInstance().advertisementLoadingSt(activity)
                }
            }
        }
    }
