...
object StLoadTranslationAd {
    private val adBase = AdBase.getTranslationInstance()
        ...
        vpnNativeAds.withAdListener(object : AdListener() {
          ...
          """"
            ...
            override fun onAdLoaded() {
                super.onAdLoaded()
                KLog.d(logTagSt, "translation--加载vpn原生广告成功")
            ...
            override fun onAdOpened() {
                super.onAdOpened()
                KLog.d(logTagSt, "translation--点击vpn原生广告")
     ...
     * 设置展示translation原生广告
     */
    fun setDisplayTranslationNativeAdSt(activity: AppCompatActivity, binding: ActivityTranslationBinding) {
        activity.runOnUiThread {
            adBase.appAdDataSt?.let { adData ->
                if (adData is NativeAd && !adBase.whetherToShowSt && activity.lifecycle.currentState == Lifecycle.State.RESUMED) {
                    ...
                    binding.translationAdSt = true
                    recordNumberOfAdDisplaysSt()
                    adBase.whetherToShowSt = true
                    App.nativeAdRefreshSt = false
                    adBase.appAdDataSt = null
                    KLog.d(logTagSt, "translation-原生广告--展示")
                    //重新缓存
                    AdBase.getTranslationInstance().advertisementLoadingSt(activity)
                }
            }
        }
    }
